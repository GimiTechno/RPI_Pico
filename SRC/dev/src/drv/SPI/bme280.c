...
static void read_compensation_parameters(void);
...
uint8_t bme280_init(void)
{
    ...
    // BME280
    if(chipid != CHIP_ID_BMP280)
    {
        printf("BME280(Chip ID = 0x%X) ... Temp,Humidity,Pressure\n",chipid);
    }else{ // BMP280
        printf("BMP280(Chip ID = 0x%X) ... Temp,Pressure\n",chipid);
    }

    read_compensation_parameters();

    // BME280 Only(BMP280 Not Support Humidity)
    // if(chipid != CHIP_ID_BMP280)
    {
